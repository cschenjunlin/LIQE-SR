...
def train(model, best_result, best_epoch, srcc_dict):
    ...
    for step in range(num_steps_per_epoch):
        ...
        # weighted total loss
        total_loss = 0
        loss_weights = [1, 1, pseudo_weight]
    ...
    if (epoch >= 0):
        ...
        srcc3 = eval_pseudo_label(pl_val_loader, phase='val', dataset='fakelabel')
        srcc33 = eval_pseudo_label(pl_test_loader, phase='test', dataset='fakelabel')

        quality_result['val'] = {'koniq10k': srcc1, 'pipal': srcc2, 'fakelabel': srcc3}
        quality_result['test'] = {'koniq10k': srcc11, 'pipal': srcc22, 'fakelabel': srcc33}
...
def eval_pseudo_label(loader, phase, dataset):
    model.eval()
    srcc = 0
    num_batches = 0
...
for session in range(0, 1):
    ...
    # avg
    srcc_dict = {'koniq10k': 0.0, 'pipal': 0.0, 'fakelabel': 0.0}

    # quality
    srcc_dict1 = {'koniq10k': 0.0, 'pipal': 0.0, 'fakelabel': 0.0}

    # splits file of datasets
    koniq10k_train_csv = os.path.join('../IQA_Database/koniq-10k/splits2', str(session+1), 'koniq10k_train.txt')
    koniq10k_val_csv = os.path.join('../IQA_Database/koniq-10k/splits2', str(session+1), 'koniq10k_val.txt')
    koniq10k_test_csv = os.path.join('../IQA_Database/koniq-10k/splits2', str(session+1), 'koniq10k_test.txt')

    pipal_train_csv = os.path.join('../IQA_Database/PIPAL/splits2', str(session+1), 'pipal_train.txt')
    pipal_val_csv = os.path.join('../IQA_Database/PIPAL/splits2', str(session+1), 'pipal_val.txt')
    pipal_test_csv = os.path.join('../IQA_Database/PIPAL/splits2', str(session+1), 'pipal_test.txt')
    ...
    pipal_set = '../IQA_Database/PIPAL/Distortion/'
