...
session = str(1)
ckpt = f'./checkpoints/weightedLoss0.01/{session}/quality_best_ckpt.pt'
...
def liqe_test_img(img_path):
    ...
    """
    if I.size[1] < 224 or I.size[0] < 224:
        I = I.resize((max(224, I.size[0]), max(224, I.size[1])))
    """
    I = preprocess2(I)
    I = I.unsqueeze(0)
...
def liqe_test_csv():
    for idx, image_row in tqdm(df.iterrows(), total=df.shape[0], desc=f"Computing 2AFC scores of liqe_quality"):
        ...
        dir_path = os.path.join('/home/user/research/SR', 'MAD', f'{group}_images', 'final', pair)
